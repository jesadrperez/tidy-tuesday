...
lifetime <- tt$lifetime_earn %>%
  ...
  mutate(
    ...
    rect_color = case_when(
      ...
      race == "White" & gender == "Men" ~ "#0057ad",
      race == "Black" & gender == "Men" ~ "#007963",
      race == "Hispanic" & gender == "Men" ~ "#d97400"
      ),
    rect_label = glue(
      '{gender} {percent(gender_within_race_share, 1)}\n',
      '{dollar(lifetime$lifetime_earn, scale = 1/1000)}'
      ),
    rect_color = fct_reorder(rect_color, race_share, .desc = TRUE)
  ) %>%
  glimpse()
...
ggplot(
  ...
  geom_text(aes(label = rect_label), position = position_stack(vjust = 0.5)) +
  scale_y_continuous(expand = c(0,0), labels = percent_format(1)) +
  scale_fill_identity() +
  facet_grid(~race_label, scales = "free_x", space = "free_x",  switch = "y") +
  labs(
    title = "This is the Title",
    subtitle = "Average lifetime earnings by race and gender in thousand of dollars",
    y = NULL,
    x = NULL,
    caption = 'Source: Urban Institute "Nine Charts about Wealth Inequality in America"'
  ) +
